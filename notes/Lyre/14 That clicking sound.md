...
function* simpleSample(duration, samplingRate, fadeFraction, generatorOfFours) {
  ...
  for (let n = 0; n < totalSamples; n = n + 1) {
    ...
    ??? // If we are in the first 40% ?
      const amplitude = ((n + 1) / fadeSamples);
      sample = sample * amplitude; // Apply fade-in
    ??? // Else if we are in the last 40% ?
      const amplitude = ((totalSamples - n - 1) / fadeSamples);
      sample = sample * amplitude; // Apply fade-out
